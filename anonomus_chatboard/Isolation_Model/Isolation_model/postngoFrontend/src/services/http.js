...
export const getPosts = async (userInfo) => {
  try {
    const response = await axios.get(
      `${process.env.REACT_APP_BACKEND_URL}/posts?cname=${userInfo.className}&role=${userInfo.role}&adminuser=${userInfo.createdBy}`
    );
...
export const reportPost = async (postId, reportData) => {
  try {
    const response = await axios.post(
      `${process.env.REACT_APP_BACKEND_URL}/posts/reports`,
      reportData
    );
